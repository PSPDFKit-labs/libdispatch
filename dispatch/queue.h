...
/*!
 * @typedef dispatch_queue_handle_t
 *
 * @abstract
 * A platform-specific handle--on Linux, this is a file descriptor--that is
 * signalled when there are pending tasks on a dispatch queue.
 */
typedef int dispatch_queue_handle_t;

/*!
 * @function dispatch_get_main_queue_handle_np
 *
 * @abstract
 * On Linux, returns a file descriptor that becomes readable when there are
 * pending tasks on the main dispatch queue.
 *
 * @discussion
 * When this file descriptor becomes readable, call
 * dispatch_main_queue_drain_np() to perform the pending tasks.
 *
 * @availability
 * Linux only.
 *
